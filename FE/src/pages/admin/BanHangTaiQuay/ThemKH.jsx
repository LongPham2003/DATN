...
export default function ThemKH({ idHoaDon, onadd }) {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:8080/api/khachhang/search", {
        params: {
          pageNumber: trangHienTai, keyword: keyword, trangThai: trangThai !== null ? trangThai : undefined
        }
      })
      .then(async (res) => {
        const data = res.data;
  ...
  // thêm khách hhangfg vào hóa đơn
  const addKhachHangVaoHoaDon = async (idKH) => {
    try {
      await axios.post(`http://localhost:8080/api/hoadon/${idHoaDon}/addkhachhang/${idKH}`);
    ...
    <div className="rounded bg-white p-4 shadow">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border border-gray-300 bg-white">
          ...
          {nhanvien.map((item, index) => (<tr key={index}>
            ...
            <td className="mx-auto flex justify-center border-b px-4 py-2 text-center">
              <button
                className={`px-4 py-2 rounded border-2 ${item.trangThai ? "border-green-400 text-green-400" : "border-red-600 text-red-600"}`}
              >
            ...
            <td>
              <button
                onClick={() => addKhachHangVaoHoaDon(item.id)}
      ...
      <div className="mr-14 mt-4 flex justify-center">
        <ReactPaginate
          ...
          previousLinkClassName={"px-3 py-1 border border-gray-300 rounded-full hover:bg-gray-200 transition duration-200"}
          nextClassName={"mx-1"}
          nextLinkClassName={"px-3 py-1 border border-gray-300 rounded-full hover:bg-gray-200 transition duration-200"}
          breakClassName={"mx-1"}
          breakLinkClassName={"px-3 py-1 border border-gray-300 rounded-full hover:bg-gray-200 transition duration-200"}
          pageClassName={"mx-1"}
          pageLinkClassName={"px-3 py-1 border-b border-green-300 rounded-full hover:bg-green-500 transition duration-200"}
